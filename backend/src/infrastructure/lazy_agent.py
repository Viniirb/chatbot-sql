...
from sqlalchemy import create_engine, text, inspect as sqlalchemy_inspect, exc, event
...
class LazyLlamaIndexChatAgent(IChatAgent):
    ...
    def _create_agent_in_loop(self) -> ReActAgent:
        ...
        sql_query_tool = QueryEngineTool.from_defaults(
            ...
            name="sql_query_tool",
            description="Ferramenta para mostrar dados do banco em texto/tabela. Use para responder perguntas sobre pessoas, dados, informações do banco. Só gere documentos (PDF, Excel, CSV) se o usuário pedir explicitamente. Priorize mostrar os dados diretamente quando o pedido for genérico.",
        )
        ...
        def generate_data_pdf(sql_query: str, title: str = "Relatorio de Dados") -> str:
            try:
                ...
                with engine.connect() as conn:
                    try:
                        ...
                        try:
                            ...
                            tables = []
                        logger.warning(
                            "SQL ProgrammingError while executing query for PDF: %s", pe
                        )
                        return f"Erro ao executar a consulta SQL: parece que a tabela ou coluna não existe. Detalhe do erro: {pe}. Tabelas disponíveis: {', '.join(tables) if tables else 'desconhecidas'}."
                ...
                from src.infrastructure.sql_exporters.data_pdf_exporter import (
                    DataPdfExporter,
                )
                ...
                if not os.path.exists(filepath):
                    logger.warning(
                        "Arquivo PDF gerado não encontrado após escrita: %s", filepath
                    )
                    return "Percebi que tentei gerar o PDF, mas o arquivo não foi encontrado. Deseja que eu tente gerar o arquivo novamente?"
        ...
        def generate_data_excel(
            sql_query: str, title: str = "Relatorio de Dados"
        ) -> str:
            try:
                prefix = "📄 " if TOOL_EMOJI else ""
                ...
                with engine.connect() as conn:
                    try:
                        ...
                        try:
                            ...
                            tables = []
                        logger.warning(
                            "SQL ProgrammingError while executing query: %s", pe
                        )
                        return f"Erro ao executar a consulta SQL: parece que a tabela ou coluna não existe. Detalhe do erro: {pe}. Tabelas disponíveis: {', '.join(tables) if tables else 'desconhecidas'}."
                    rows = result.fetchall()
                    columns = result.keys()
                ...
                logger.info(f"📊 Dados obtidos: {len(data)} registros")
                from src.infrastructure.sql_exporters.data_excel_exporter import (
                    DataExcelExporter,
                )
                ...
                if not os.path.exists(filepath):
                    logger.warning(
                        "Arquivo Excel gerado não encontrado após escrita: %s", filepath
                    )
                    return "Percebi que tentei gerar o Excel, mas o arquivo não foi encontrado. Deseja que eu tente gerar o arquivo novamente?"
        ...
        def generate_data_csv(sql_query: str, title: str = "Relatorio de Dados") -> str:
            try:
                ...
                with engine.connect() as conn:
                    try:
                        ...
                        try:
                            ...
                            tables = []
                        logger.warning(
                            "SQL ProgrammingError while executing query for CSV: %s", pe
                        )
                        return f"Erro ao executar a consulta SQL: parece que a tabela ou coluna não existe. Detalhe do erro: {pe}. Tabelas disponíveis: {', '.join(tables) if tables else 'desconhecidas'}."
                    rows = result.fetchall()
                    columns = result.keys()
                ...
                logger.info(f"📊 Dados obtidos: {len(data)} registros")
                from src.infrastructure.sql_exporters.data_csv_exporter import (
                    DataCsvExporter,
                )
                ...
                try:
                    ...
                    filename = (
                        f"relatorio_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
                    )
                ...
                if not os.path.exists(filepath):
                    logger.warning(
                        "Arquivo CSV gerado não encontrado após escrita: %s", filepath
                    )
                    return "Percebi que tentei gerar o CSV, mas o arquivo não foi encontrado. Deseja que eu tente gerar o arquivo novamente?"
        ...
        pdf_tool = FunctionTool.from_defaults(
            ...
            description="Gera um arquivo PDF com dados de uma consulta SQL. Só use esta ferramenta se o usuário pedir explicitamente para exportar ou baixar os dados em PDF. Sempre gere um novo arquivo quando solicitado pelo usuário. Nunca gere documentos sem solicitação clara do usuário.",
        )

        excel_tool = FunctionTool.from_defaults(
            fn=generate_data_excel,
            name="generate_excel_tool",
            description="Gera um arquivo Excel com dados de uma consulta SQL. Só use esta ferramenta se o usuário pedir explicitamente para exportar ou baixar os dados em Excel. Sempre gere um novo arquivo quando solicitado pelo usuário. Nunca gere documentos sem solicitação clara do usuário.",
        )

        csv_tool = FunctionTool.from_defaults(
            fn=generate_data_csv,
            name="generate_csv_tool",
            description="Gera um arquivo CSV com dados de uma consulta SQL. Só use esta ferramenta se o usuário pedir explicitamente para exportar ou baixar os dados em CSV. Sempre gere um novo arquivo quando solicitado pelo usuário. Nunca gere documentos sem solicitação clara do usuário.",
        )
                        ...
                        if text_parts:
                            ...
                            if url_match and not markdown_match:
                                ...
                                try:
                                    ...
                                    logger.debug(
                                        "Falha ao validar URL de download: %s", url
                                    )
                                result_text = (
                                    f"📥 [Clique aqui para baixar o arquivo]({url})"
                                )
                            return result_text
