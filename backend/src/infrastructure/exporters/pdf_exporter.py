...
class PdfExporter(IExporter):
    def export(self, session: Session) -> bytes:
        ...
        pdf.cell(
            0, 10, f"Sessao de Chat: {session.session_id.value}", ln=True, align="C"
        )

        pdf.set_font("Arial", "", 10)
        pdf.cell(
            0, 8, f"Criada em: {self._format_datetime(session.created_at)}", ln=True
        )
        pdf.cell(
            0,
            8,
            f"Ultima atividade: {self._format_datetime(session.last_activity)}",
            ln=True,
        )
        pdf.ln(5)
        ...
        if session.active_dataset:
            ...
            pdf.set_font("Arial", "", 9)
            pdf.multi_cell(
                0, 5, f"Consulta: {self._sanitize_text(session.active_dataset.query)}"
            )
            pdf.cell(0, 6, f"Registros: {session.active_dataset.row_count}", ln=True)
            pdf.multi_cell(
                0, 5, f"Colunas: {', '.join(session.active_dataset.columns)}"
            )
        ...
        pdf.cell(
            0,
            6,
            f"Exportado em: {self._format_datetime(datetime.now())}",
            ln=True,
            align="C",
        )
