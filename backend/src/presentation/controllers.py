...
class ChatController:
    def __init__(
        self,
        process_query_use_case: IProcessQueryUseCase,
    ...
    async def process_query(
        self, request: QueryRequest, request_id: Optional[str] = None
    ) -> JSONResponse:
        query_text = request.prompt or request.query
        if not query_text:
            raise HTTPException(
                status_code=400, detail="Campo 'query' ou 'prompt' é obrigatório"
            )

        print(
            f"💬 {query_text[:80]}..." if len(query_text) > 80 else f"💬 {query_text}"
        )
        if query_text.strip().startswith(("Assistente:", "Assistant:")):
            raise HTTPException(
                status_code=400,
                detail="Mensagem inválida: não envie mensagens do assistente",
            )
...
class SessionController:
    def __init__(
        self,
        session_management_use_case: ISessionManagementUseCase,
        schema_service: SchemaService = None,
    ):
    ...
    def create_session(self) -> JSONResponse:
        ...
        return JSONResponse(
            {"session_id": response.session_id, "message": "Sessão criada com sucesso"}
        )
    ...
    def update_session_stats(
        self, session_id: str, request: SessionStatsUpdateRequest
    ) -> JSONResponse:
        try:
            ...
            response = self._session_management_use_case.update_session_stats(
                session_id, use_case_request
            )
            return JSONResponse(
                {
            ...
            raise HTTPException(
                status_code=500, detail=f"Erro ao atualizar estatísticas: {str(e)}"
            )
...
class ExportController:
    def __init__(
        self,
        export_use_case: IExportSessionUseCase,
        schema_service: SchemaService = None,
    ):
    ...
    def export_session(self, session_id: str, request: ExportRequest):
        ...
        payload_id = (
            ...
            if has_payload and isinstance(request.session, dict)
            else None
        )
        print(
            f"📤 EXPORT_REQUEST session_id={session_id} format={request.format} payload_present={has_payload} payload_id={payload_id} — {now} — DEBUG",
            flush=True,
        )
        ...
        try:
            ...
            raise HTTPException(
                status_code=400,
                detail="Formato inválido. Formatos válidos: pdf, json, txt",
            )

        try:
            result = self._export_use_case.execute(
                session_id, export_format, session_payload=request.session
            )
            headers = {
                "Content-Disposition": f'attachment; filename="{result.filename}"'
            }
            return Response(
                content=result.content, media_type=result.content_type, headers=headers
            )
        except ValueError as e:
            raise HTTPException(status_code=404, detail=str(e))
        except Exception as e:
            raise HTTPException(
                status_code=500, detail=f"Erro ao exportar sessão: {str(e)}"
            )
...
def setup_routes(
    ...
    async def log_requests(request, call_next):
        ...
        print(
            f"{status_emoji} {request.method} {request.url.path} - {response.status_code} - {timestamp}"
        )
        return response
    ...
    def get_server_status():
        return JSONResponse(
            {"status": "ok", "service": "Chatbot SQL Agent", "dbStatus": "connected"}
        )
    ...
    async def ask_agent(body: QueryRequest, fastapi_request: Request):
        ...
        print("🗃️ ROTA /ask CHAMADA!", flush=True)
        print(f"📫 Request: {body}", flush=True)

        client_req_id = fastapi_request.headers.get(
            "x-request-id"
    ...
    def update_session_stats_route(session_id: str, request: SessionStatsUpdateRequest):
    ...
    def export_session_route(session_id: str, request: ExportRequest):
    ...
    def sync_session(session_id: str, request: dict):
        session_service = (
            session_controller._session_management_use_case._session_service
        )
        ...
        session.updated_at = (
            datetime.fromisoformat(request["updatedAt"])
        ...
        client_query_count = request.get("queryCount")
        if client_query_count is not None:
            try:
                session.stats.query_count = max(
    ...
    async def cancel_request(request_id: str):
        ...
        try:
            ...
            return JSONResponse(
                {"status": "cancellation_requested", "request_id": request_id}
            )
        except Exception as e:
            raise HTTPException(
                status_code=500, detail=f"Failed to cancel request: {e}"
            )
